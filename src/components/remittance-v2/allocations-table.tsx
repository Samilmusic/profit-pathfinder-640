                ...
                {q.data!.map((a: Record<string, unknown> & { buy?: { doc_no?: string | null } | null }) => (
                  <tr key={a.id} className="border-t">
                    ...
                    <td className="py-2 pr-3">
                      {a.buy ? (
                        <span className="font-mono text-xs">{a.buy.doc_no ?? String(a.buy_id).slice(0, 8)}</span>
                      ) : a.lot_id ? (
                    ...
                    <td className="py-2 pr-3">{fmt(a.allocated_amount)} {a.currency}</td>
                    <td className="py-2 pr-3">{a.frozen_cost_amount != null ? `${fmt(a.frozen_cost_amount)} ${a.frozen_cost_currency ?? ""}` : "—"}</td>
                    <td className="py-2 pr-3">{a.frozen_spread_profit_aed != null ? fmt(a.frozen_spread_profit_aed) : "—"}</td>
                    <td className="py-2 pr-3">{a.frozen_commission_aed != null ? fmt(a.frozen_commission_aed) : "—"}</td>
                    <td className="py-2 pr-3">{a.frozen_total_profit_aed != null ? fmt(a.frozen_total_profit_aed) : "—"}</td>
                    <td className="py-2 pr-3"><Badge variant="secondary">{String(a.status)}</Badge></td>
                    <td className="py-2 pr-3">
                      <Badge variant={String(a.entry_kind) === "normal" ? "outline" : "destructive"}>{String(a.entry_kind)}</Badge>
                      {a.parent_allocation_id ? (
                        <div className="mt-1 text-[10px] text-muted-foreground">reverses {String(a.parent_allocation_id).slice(0, 8)}</div>
                      ) : null}
                    </td>
                  </tr>
                ))}
