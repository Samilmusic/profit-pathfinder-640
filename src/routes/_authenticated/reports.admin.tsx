  ...
  return (
    <div className="p-4 sm:p-6 space-y-4">
      ...
      <UIAlert>
        ...
        <AlertDescription>
          This module does not alter the accounting engine. Dismissing an alert stores metadata only.
        </AlertDescription>
      </UIAlert>

      <Tabs defaultValue="health" className="space-y-4">
        <TabsList className="flex-wrap h-auto">
          <TabsTrigger value="health"><Gauge className="h-4 w-4 mr-1.5" />System</TabsTrigger>
          <TabsTrigger value="reporting"><Database className="h-4 w-4 mr-1.5" />Reporting</TabsTrigger>
          <TabsTrigger value="alerts"><Bell className="h-4 w-4 mr-1.5" />Alerts</TabsTrigger>
          <TabsTrigger value="perf"><ServerCog className="h-4 w-4 mr-1.5" />Performance</TabsTrigger>
          <TabsTrigger value="registry"><FileText className="h-4 w-4 mr-1.5" />Export Center</TabsTrigger>
        </TabsList>

        <TabsContent value="health"><SystemHealthPanel /></TabsContent>
        <TabsContent value="reporting"><ReportingHealthPanel /></TabsContent>
        <TabsContent value="alerts"><AlertsPanel /></TabsContent>
        <TabsContent value="perf"><PerformancePanel /></TabsContent>
        <TabsContent value="registry"><ExportCenterPanel /></TabsContent>
      </Tabs>
    </div>
  );
}
...
function SystemHealthPanel() {
  const q = useQuery({ queryKey: ["report_system_health"], queryFn: fetchSystemHealth, staleTime: 60_000 });
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      <StatCard label="Database" value={d.db.database} subtitle={`Postgres ${d.db.server_version}`} icon={<Database className="h-4 w-4" />} />
      <StatCard label="DB size" value={formatBytes(d.db.size_bytes)} subtitle={d.db.timezone} />
      <StatCard label="Tables / Views" value={`${d.table_count} / ${d.view_count}`} subtitle={`${d.report_function_count} report functions`} />
      <StatCard label="Auth users" value={d.auth_users} subtitle={`${d.user_roles_count} role bindings`} />
      <StatCard label="Materialized views" value={d.matviews.length} subtitle={d.matviews.length ? d.matviews.map(m => m.name).join(", ") : "none"} />
      <StatCard label="pg_stat_statements" value={d.pg_stat_statements_available ? "enabled" : "disabled"} />
      ...
      <Card>
        ...
        <CardContent className="text-sm space-y-1">
          ...
          <Row label="Invalid" value={d.data_quality.invalid} tone={d.data_quality.invalid > 0 ? "warning" : undefined} />
        </CardContent>
      </Card>

      <Card>
        <CardHeader className="pb-2"><CardTitle className="text-sm">Last reconciliation</CardTitle></CardHeader>
        <CardContent className="text-sm">
          {d.last_reconciliation.run_at
            ? <>Last logged {new Date(d.last_reconciliation.run_at).toLocaleString()} · {d.last_reconciliation.row_count} entries</>
            : <span className="text-muted-foreground">No reconciliation events logged yet.</span>}
        </CardContent>
      </Card>

      <Card className="md:col-span-2 xl:col-span-3">
        <CardHeader className="pb-2"><CardTitle className="text-sm">Extensions</CardTitle></CardHeader>
        <CardContent className="flex flex-wrap gap-2">
          {d.extensions.map(e => (
            <Badge key={e.name} variant="outline" className="font-mono text-[11px]">{e.name} {e.version}</Badge>
          ))}
        </CardContent>
      </Card>
      ...
      <Card className="md:col-span-2 xl:col-span-3">
        <CardHeader className="pb-2 flex flex-row items-center justify-between">
          ...
          <Badge variant={d.pg_cron_available ? "default" : "outline"}>{d.pg_cron_available ? "available" : "unavailable"}</Badge>
        </CardHeader>
        <CardContent className="text-sm">
          {Array.isArray(d.background_jobs) && d.background_jobs.length > 0
            ? <pre className="text-xs overflow-auto max-h-40">{JSON.stringify(d.background_jobs, null, 2)}</pre>
...
function ReportingHealthPanel() {
  const q = useQuery({ queryKey: ["report_reporting_health"], queryFn: fetchReportingHealth, staleTime: 60_000 });
  if (q.isLoading) return <Skeleton className="h-64" />;
  if (q.error) return <ErrorBox error={q.error} />;
  const d = q.data!;
  const doExport = () => {
    const csv = buildCsv(
      ...
      d.meta,
      { quality_mode: "n/a", rows_included: d.summary.included_in_executive, rows_excluded: d.summary.excluded_in_executive },
    );
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-4 md:grid-cols-3">
        <StatCard label="Rows processed" value={d.summary.total} subtitle={`${d.summary.included_in_executive} included in Executive`} />
        <StatCard label="Rows excluded" value={d.summary.excluded_in_executive} subtitle="quality_mode=exclude_invalid" tone={d.summary.excluded_in_executive > 0 ? "warning" : undefined} />
      ...
      <Card>
        <CardHeader className="pb-2 flex flex-row items-center justify-between">
          ...
          <Button size="sm" variant="outline" onClick={doExport}><Download className="h-4 w-4 mr-1.5" />CSV</Button>
        </CardHeader>
        <CardContent>
          <div className="text-sm">
            <div className="grid grid-cols-5 gap-2 font-medium text-xs text-muted-foreground border-b pb-1.5">
              <div>Source</div><div className="text-right">Total</div><div className="text-right">Valid</div><div className="text-right">Suspicious</div><div className="text-right">Invalid</div>
            </div>
            {d.by_source.map(r => (
              <div key={r.source_table} className="grid grid-cols-5 gap-2 py-1 text-sm border-b last:border-b-0">
      ...
      <Card>
        <CardHeader className="pb-2"><CardTitle className="text-sm">Report query stats</CardTitle></CardHeader>
        <CardContent>
...
function AlertsPanel() {
  ...
  const hist = useQuery({ queryKey: ["alert_history"], queryFn: () => fetchAlertHistory(200), staleTime: 60_000 });
  ...
  function doExport() {
    ...
    const csv = buildCsv(
      ...
      filtered.map(a => [a.key, a.level, a.category, a.title, a.message, a.raised_at, a.dismissed ? "yes" : "no"]),
      q.data.meta,
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-4 grid-cols-2 sm:grid-cols-4">
        ...
        <StatCard label="Critical" value={c.critical} tone={c.critical > 0 ? "critical" : undefined} />
      ...
      <div className="flex flex-wrap items-center gap-2">
        {(["all", "critical", "warning", "info"] as const).map(l => (
          <Button key={l} size="sm" variant={levelFilter === l ? "default" : "outline"} onClick={() => setLevelFilter(l)}>{l}</Button>
        ))}
        <Button size="sm" variant={includeDismissed ? "default" : "outline"} onClick={() => setIncludeDismissed(v => !v)}>
          {includeDismissed ? "Hide dismissed" : "Show dismissed"}
        </Button>
        <div className="flex-1" />
        <Button size="sm" variant="outline" onClick={() => q.refetch()}><RefreshCw className="h-4 w-4 mr-1.5" />Refresh</Button>
        <Button size="sm" variant="outline" onClick={doExport}><Download className="h-4 w-4 mr-1.5" />CSV</Button>
      </div>

      {filtered.length === 0
        ? <Card><CardContent className="py-6 text-sm text-muted-foreground text-center">No alerts match the current filter.</CardContent></Card>
        : (
                ...
                <CardContent className="py-3 flex flex-col sm:flex-row items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {a.dismissed && a.dismissed_meta?.reason && (
                      <div className="text-[11px] text-muted-foreground mt-1">Dismissed: {a.dismissed_meta.reason}</div>
                    )}
                  </div>
                  <div className="flex gap-2">
                    {a.dismissed
                      ? <Button size="sm" variant="outline" onClick={() => onRestore(a)}><Undo2 className="h-4 w-4 mr-1.5" />Restore</Button>
                      : <Button size="sm" variant="outline" onClick={() => onDismiss(a)}>Dismiss</Button>}
                  </div>
                </CardContent>
              </Card>
            ))}
      ...
      <Card>
        <CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><History className="h-4 w-4" />Dismiss history</CardTitle></CardHeader>
        <CardContent>
          {hist.data && hist.data.rows.length > 0
            ? (
              <div className="text-sm max-h-64 overflow-auto">
                {hist.data.rows.map(h => (
                  <div key={h.id} className="grid grid-cols-4 gap-2 py-1 border-b last:border-b-0 text-xs">
...
function PerformancePanel() {
  const q = useQuery({ queryKey: ["report_slow_queries", 25], queryFn: () => fetchSlowQueries(25), staleTime: 60_000 });
  const health = useQuery({ queryKey: ["report_system_health"], queryFn: fetchSystemHealth, staleTime: 60_000 });
  if (q.isLoading) return <Skeleton className="h-64" />;
  if (q.error) return <ErrorBox error={q.error} />;
  const d = q.data!;
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard label="pg_stat_statements" value={d.available ? "available" : "unavailable"} icon={<Database className="h-4 w-4" />} />
      ...
      <Card>
        <CardHeader className="pb-2"><CardTitle className="text-sm">Known limitations</CardTitle></CardHeader>
    ...
    <div className="space-y-4">
      <Card>
        ...
        <CardContent className="text-sm space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            {d.export_formats.map(f => <Badge key={f} variant="outline">{f.toUpperCase()}</Badge>)}
          </div>
          <div className="pt-2 text-muted-foreground">Metadata attached to every export:</div>
          <div className="flex flex-wrap gap-2">
            {d.metadata_fields.map(f => <Badge key={f} variant="outline" className="font-mono text-[10px]">{f}</Badge>)}
          </div>
          <div className="pt-2 text-xs text-muted-foreground">
            PDF and Print use the browser print dialog on each report page; Excel is served as CSV that Excel opens natively.
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader className="pb-2 flex flex-row items-center justify-between">
          ...
          <Button size="sm" variant="outline" onClick={exportRegistry}><Download className="h-4 w-4 mr-1.5" />CSV</Button>
        </CardHeader>
        <CardContent>
            ...
            {d.reports.map(r => {
              ...
              return (
                <div key={r.key} className="border rounded-md p-3 flex items-start gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium capitalize">{r.key.replaceAll("_", " ")}</div>
                    <div className="font-mono text-[11px] text-muted-foreground truncate">{r.rpc}</div>
                    <div className="flex flex-wrap gap-1 mt-1">
                      <Badge variant="outline" className="text-[10px]">v{r.version}</Badge>
                      <Badge variant="outline" className="text-[10px]">Slice {r.slice}</Badge>
                      <Badge variant="secondary" className="text-[10px]">read-only</Badge>
                    </div>
                  </div>
                  {!isDynamic && (
                    <Link to={r.route} className="text-primary text-xs inline-flex items-center gap-1">
...
// ─────────────────────────────────────────────
// Shared bits
function StatCard({ label, value, subtitle, icon, tone }: { label: string; value: React.ReactNode; subtitle?: React.ReactNode; icon?: React.ReactNode; tone?: "warning" | "critical" }) {
  const border =
    tone === "critical" ? "border-destructive/60" :
    tone === "warning"  ? "border-yellow-500/50" : "";
  return (
...
function Row({ label, value, tone }: { label: string; value: React.ReactNode; tone?: "warning" }) {
  return (
    <div className="flex justify-between">
      ...
      <span className={tone === "warning" ? "text-yellow-600 dark:text-yellow-400 font-medium" : "font-medium"}>{value}</span>
    </div>
  );
}

function SlowTable({ rows }: { rows: { query: string; calls: number; total_ms: number; mean_ms: number; max_ms: number; rows: number }[] }) {
  if (!rows || rows.length === 0) return <div className="text-sm text-muted-foreground">No data collected.</div>;
  return (
    <div className="text-xs">
      ...
      {rows.slice(0, 25).map((r, i) => (
        <div key={i} className="grid grid-cols-12 gap-2 py-1 border-b last:border-b-0">
          <div className="col-span-6 font-mono truncate" title={r.query}>{r.query}</div>
