...
-- Version: 1.0.0
-- Adds versioned reporting surface for cash position, forecast, currency
-- exposure, and bank-account analytics. No workflow / settlement / allocation
-- / posting / close / reconciliation / feature-flag changes.
-- ============================================================================

-- ---------------------------------------------------------------------------
-- Helper view: authoritative live account balance from ledger.
-- ---------------------------------------------------------------------------
...
SELECT
  ...
  a.bank_name,
  a.holder_type,
  a.holder_customer_id,
  a.holder_person_name,
  a.is_active,
  a.opening_balance,
...
DECLARE
  ...
  v_largest jsonb; v_dormant jsonb;
  v_reserved jsonb; v_pending jsonb;
  v_expected_in jsonb; v_expected_out jsonb;
  v_kpis jsonb;
  v_largest_move jsonb;
BEGIN
  ...
  -- filtered balances
  WITH b AS (
    SELECT * FROM public.v_account_balances
  ...
  b_aed AS (
    SELECT *,
      CASE
        WHEN currency = 'AED' THEN balance
  ...
  SELECT
    jsonb_agg(row_to_json(t)) FILTER (WHERE t.currency IS NOT NULL)
  INTO v_by_currency
  FROM (
    SELECT
      currency,
      SUM(balance)      AS held,
      SUM(balance_aed)  AS held_aed,
  ...
  SELECT jsonb_agg(row_to_json(t)) INTO v_by_owner
  FROM (
    SELECT account_owner AS owner,
           COUNT(*) AS account_count,
    ...
    FROM b GROUP BY account_owner ORDER BY account_owner
  ) t;
  ...
  SELECT jsonb_agg(row_to_json(t)) INTO v_by_account
  FROM (
    SELECT account_id, account_name, currency, account_owner, account_type::text AS account_type,
           bank_name, balance, last_activity_at, entries_30d, entries_7d
    FROM b
    ORDER BY currency, balance DESC
  ) t;

  -- largest balances (top 10 by AED-equivalent)
  WITH b AS (
    SELECT v.*,
      CASE
        WHEN currency='AED' THEN balance
  ...
  SELECT jsonb_agg(row_to_json(t)) INTO v_largest
  FROM (
    SELECT account_id, account_name, currency, account_owner, balance, balance_aed
    FROM b
    WHERE balance <> 0
    ORDER BY balance_aed DESC NULLS LAST, balance DESC
    LIMIT 10
  ) t;

  -- dormant accounts: no activity in 60 days but non-zero balance
  SELECT jsonb_agg(row_to_json(t)) INTO v_dormant
  FROM (
    SELECT account_id, account_name, currency, account_owner, balance,
           last_activity_at,
  ...
  -- Reserved cash: v2 remittances in obligation states, per settlement_currency
  SELECT jsonb_agg(row_to_json(t)) INTO v_reserved
  FROM (
    SELECT settlement_currency AS currency,
  ...
  -- Pending cash: draft remittances
  SELECT jsonb_agg(row_to_json(t)) INTO v_pending
  FROM (
    SELECT settlement_currency AS currency,
           SUM(COALESCE(settlement_amount, 0)) AS amount,
           COUNT(*) AS count
    FROM public.remittances
  ...
  -- Expected inflows (30d horizon): draft remittances customer payments
  SELECT jsonb_agg(row_to_json(t)) INTO v_expected_in
  FROM (
    SELECT customer_payment_currency AS currency,
           SUM(customer_payment_amount) AS amount,
           COUNT(*) AS count
    FROM public.remittances
    WHERE workflow_state IN ('draft','funds_received')
      AND customer_payment_currency IS NOT NULL
      AND customer_payment_amount IS NOT NULL
      AND (_currency IS NULL OR customer_payment_currency = _currency)
    GROUP BY customer_payment_currency
  ) t;

  -- Expected outflows (30d horizon): remittances awaiting settlement
  SELECT jsonb_agg(row_to_json(t)) INTO v_expected_out
  FROM (
    SELECT settlement_currency AS currency,
           SUM(settlement_amount) AS amount,
           COUNT(*) AS count
  ...
  -- Largest daily movement across [v_from, v_to]
  SELECT to_jsonb(t) INTO v_largest_move
  FROM (
    SELECT entry_date,
  ...
  m AS (
    SELECT
      SUM(ABS(l.amount)) AS total_move,
      AVG(daily.total) AS avg_daily_bal_change
    FROM public.ledger_entries l
    JOIN public.accounts a ON a.id=l.account_id
    LEFT JOIN LATERAL (
      SELECT SUM(ABS(l2.amount)) AS total
      FROM public.ledger_entries l2
      WHERE l2.entry_date = l.entry_date
    ) daily ON true
    WHERE l.entry_date BETWEEN v_from AND v_to
      AND a.node_type='currency_account'
      AND (_currency IS NULL OR l.currency=_currency)
  ...
  RETURN jsonb_build_object(
    'meta', jsonb_build_object(
      'report_key', 'treasury_overview',
      'report_version', '1.0.0',
      'generated_at', NOW(),
      'data_cutoff', NOW(),
      'generated_by_version', 'phase6-slice5'
    ),
    'quality_mode', v_mode,
    'date_from', v_from,
    'date_to', v_to,
...
-- ---------------------------------------------------------------------------
-- 2. report_treasury_cashflow — daily/weekly/monthly in/out/net + forecast
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  WITH raw AS (
    SELECT
      date_trunc(v_gran, l.entry_date::timestamp)::date AS bucket_start,
      l.currency,
      l.amount
    FROM public.ledger_entries l
    JOIN public.accounts a ON a.id = l.account_id
  ...
  SELECT jsonb_agg(row_to_json(t) ORDER BY t.bucket_start)
  INTO v_series
  FROM (
  ...
  -- Rolling-average forecast: mean of last N buckets' net.
  WITH daily AS (
    SELECT
      l.entry_date,
  ...
  SELECT
    jsonb_agg(jsonb_build_object(
      'bucket_start', gs.d,
      'inflow_est',  stats.avg_inflow,
      'outflow_est', stats.avg_outflow,
      'net_est',     stats.avg_net,
      'is_estimate', true
    ) ORDER BY gs.d),
    to_jsonb(stats)
  INTO v_forecast, v_stats
  FROM gs, stats;

  RETURN jsonb_build_object(
    'meta', jsonb_build_object(
      'report_key','treasury_cashflow',
      'report_version','1.0.0',
      'generated_at', NOW(),
      'data_cutoff',  NOW(),
      'generated_by_version','phase6-slice5'
...
BEGIN
  WITH held AS (
    SELECT currency, SUM(balance) AS held
    FROM public.v_account_balances
  ...
  SELECT jsonb_agg(row_to_json(t) ORDER BY t.currency)
  INTO v_rows
  FROM (
    SELECT
      h.currency,
      h.held,
      COALESCE(r.amt,0) AS reserved,
      COALESCE(p.amt,0) AS pending,
      h.held - COALESCE(r.amt,0) - COALESCE(p.amt,0) AS net_position,
      m.market_mid,
      m.snapshot_at AS market_snapshot_at,
    ...
    FROM held h
    LEFT JOIN reserved r ON r.currency = h.currency
    LEFT JOIN pending  p ON p.currency = h.currency
    LEFT JOIN mkt      m ON m.currency = h.currency
  ) t;

  -- Historical trend: daily balance change per currency over window
  WITH daily AS (
    SELECT
      l.entry_date,
      l.currency,
      SUM(l.amount) AS net_change
    FROM public.ledger_entries l
    JOIN public.accounts a ON a.id=l.account_id
    WHERE l.entry_date BETWEEN v_from AND v_to
      AND a.node_type='currency_account'
  ...
  SELECT jsonb_agg(row_to_json(t) ORDER BY t.entry_date, t.currency)
  INTO v_trend
  FROM (
...
-- ---------------------------------------------------------------------------
-- 4. report_bank_account_analytics
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  activity AS (
    SELECT
      ...
      SUM(CASE WHEN l.amount < 0 THEN -l.amount ELSE 0 END) AS outflow,
      SUM(l.amount) AS net,
      AVG(ABS(l.amount)) AS avg_tx,
      MAX(ABS(l.amount)) AS largest_tx,
  ...
  avg_bal AS (
    -- avg daily balance approximated by opening + running sum divided by day count
    SELECT
      a.id AS account_id,
      a.opening_balance + COALESCE((
        SELECT SUM(l3.amount) FROM public.ledger_entries l3
        WHERE l3.account_id=a.id AND l3.entry_date < v_from
      ),0) AS start_balance,
      (
        SELECT AVG(x.bal) FROM (
          SELECT SUM(l4.amount) OVER (ORDER BY l4.entry_date) AS bal
          FROM public.ledger_entries l4
          WHERE l4.account_id=a.id AND l4.entry_date BETWEEN v_from AND v_to
        ) x
      ) AS avg_delta
    FROM public.accounts a
  ...
  full_rows AS (
    SELECT
      r.*,
      COALESCE(ac.movements,0) AS movements,
      COALESCE(ac.inflow,0)    AS inflow,
      COALESCE(ac.outflow,0)   AS outflow,
      COALESCE(ac.net,0)       AS net_flow,
      ac.avg_tx,
      ac.largest_tx,
      ac.most_active_period,
      COALESCE(ab.start_balance,0) + COALESCE(ab.avg_delta,0) AS avg_daily_balance,
      CASE
        WHEN r.days_dormant > 60 THEN 'dormant'
        WHEN r.days_dormant > 14 THEN 'quiet'
        ELSE 'active'
...
-- ---------------------------------------------------------------------------
-- 5. report_treasury_account_detail — drill-down for one account
-- ---------------------------------------------------------------------------
...
BEGIN
  SELECT row_to_json(t) INTO v_acc
  FROM (
  ...
  SELECT jsonb_agg(row_to_json(t) ORDER BY t.entry_date)
  INTO v_series
  FROM (
  ...
  FROM (
    ...
    WHERE l.account_id = _account_id
      AND l.entry_date BETWEEN v_from AND v_to
  ...
  RETURN jsonb_build_object(
    'meta', jsonb_build_object(
      'report_key','treasury_account_detail',
      'report_version','1.0.0',
      'generated_at', NOW(),
      'data_cutoff', NOW(),
      'generated_by_version','phase6-slice5'
